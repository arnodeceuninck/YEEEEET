...
void schedule(std::vector<Library *> libraries, int maxDays, std::string filename)
{
    ...
    while (true)
    {
        ...
        queue.emplace_back(std::make_pair(libraries[index], ((maxDays - daysPassed - libraries[index]->time) *
                                                             libraries[index]->booksPerDay)));
        bools[index] = true;
    }

    outputFile << queue.size() << "\n";

    for (int lib = 0; lib < queue.size() - 1; lib++)
    {
        outputFile << queue[lib].first->ID << " " << queue[lib].second << "\n";
        int index = 0;
        for (int book = 0; book < queue[lib].second; book++)
        {
            while (queue[lib].first->books[index]->isScanned)
            {
                index++;
            }
            outputFile << queue[lib].first->books[index]->ID << " ";
            index++;
        }
        outputFile << "\n";
    }
